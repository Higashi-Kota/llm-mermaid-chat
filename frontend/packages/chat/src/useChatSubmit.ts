...
import {
  type BeautifyKeyTextInputKey,
  beautifyKeyTextList,
  type ModKey,
  type ShortcutHintLabelStyle,
...
function resolveModKey(modKey: ModKeyOption, isApple: undefined | boolean): undefined | ModKey {
  if (modKey === "meta" || modKey === "ctrl") return modKey
  if (isApple === undefined) return undefined
  return isApple ? "meta" : "ctrl"
}

function isModPressed(e: React.KeyboardEvent, resolved: undefined | ModKey): undefined | boolean {
  if (resolved === undefined) return undefined
  return resolved === "meta" ? e.metaKey : e.ctrlKey
}

function getSubmitKeys(mode: SubmitMode, resolved: ModKey): BeautifyKeyTextInputKey[] {
...
export function useChatSubmit(options: UseChatSubmitOptions): UseChatSubmitReturn {
  ...
  const instrumentedRef = React.useCallback<React.RefCallback<HTMLTextAreaElement>>(
    (node) => {
      if (!node) return
      if (!isEnabled) return
      ...
      const onNativeKeyDown = (e: KeyboardEvent) => {
        ...
        if (e.isComposing || e.key === "Process" || e.keyCode === 229 || e.which === 229) {
  ...
  const getTextareaProps = React.useCallback<UseChatSubmitReturn["getTextareaProps"]>(
    (userProps) => {
      const { onKeyDown: onKeyDownUser, ref: userRef, ...rest } = userProps ?? {}
  ...
  const shortcutHintLabels = React.useMemo<ShortcutHintLabels | undefined>(() => {
