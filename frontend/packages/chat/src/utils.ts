...
export function getUserAgentSummary(): undefined | UserAgentSummary {
  ...
  const isIos = /iphone|ipad/.test(ua) || (/mac os/.test(ua) && "ontouchend" in document)
...
export function useUserAgentSummary(
  ...
  const [uaSummary, setUaSummary] = React.useState<UserAgentSummary | undefined>(uaSummaryHint)
